...
- [X] T001 [P] Write failing unit test for the status-mapping helper in
...
- [X] T002 Implement `src/features/automations/runHistoryStatus.ts` exporting
...
- [X] T003 [P] Write failing integration test in `src/features/automations/rulesRepo.test.ts`
...
- [X] T004 Implement `listRunsForRule` in `src/features/automations/rulesRepo.ts` (move the
...
- [X] T005 Update `automationsRouter.listRunsForRule` in `src/features/automations/router.ts`
...
- [X] T006 [P] Write failing integration test in `src/features/automations/rulesRepo.test.ts`
  ...
  returns `[]`.
- [X] T007 Implement `listActionsForRun` in `src/features/automations/rulesRepo.ts` to make
  T006 pass: `SELECT * FROM automation_run_actions WHERE run_id = :runId ORDER BY position ASC`.
- [X] T008 Add `listActionsForRun` procedure to `automationsRouter` in
...
- [X] T009 [P] [US1] Add a "View runs" link per row to
...
- [X] T014 [US1] In `RunHistoryTable.tsx`, render each run's deal cell as a link to
...
- [X] T015 [US2] In `RunHistoryTable.tsx` (from T013), add an error-message row/line beneath
...
- [X] T016 [US3] Add an expand/collapse affordance per row in `RunHistoryTable.tsx` (a
...
- [X] T017 [US3] On expand, fetch that run's actions via the tRPC client
  ...
  expanded row.
- [X] T018 [US3] For an action whose `status === "error"`, show its own `errorMessage` beneath
...
- [X] T019 [P] Run `pnpm lint` and fix any Biome/ESLint findings across all files touched by
  this feature (new files listed above plus `AutomationsTable.tsx`/`AutomationsClient.tsx`).
- [X] T020 [P] Run `pnpm test:unit` and `pnpm test:integration` for the full
