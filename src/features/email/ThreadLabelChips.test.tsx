...
const catalog = [
  ...
  { id: "l3", key: "later", name: "Depois", color: "blue", order: 2 },
...
it("renders a colored chip with the catalog name for each applied key", () => {
  ...
  expect(screen.getByText("Importante")).toHaveClass("bg-red-100");
  expect(screen.getByText("Depois")).toHaveClass("bg-blue-100");
});

it("resolves the to_do token to its catalog name and color", () => {
  render(<ThreadLabelChips labels={["to_do"]} />);
  expect(screen.getByText("A fazer")).toHaveClass("bg-orange-100");
});
...
it("renders each chip uppercase (A15)", () => {
  ...
  expect(screen.getByText("Importante")).toHaveClass("uppercase");
});
