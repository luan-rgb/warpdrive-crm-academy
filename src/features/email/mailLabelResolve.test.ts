...
const catalog = [
  { key: "important", name: "Importante", color: "red" as const },
  { key: "to_do", name: "A fazer", color: "orange" as const },
...
describe("resolveMailLabelChips", () => {
  it("maps applied keys to catalog name + color classes, preserving order", () => {
    ...
    expect(chips.map((c) => c.name)).toEqual(["Newsletter", "Importante"]);
  ...
  it("matches keys case-insensitively", () => {
    ...
    expect(chips.map((c) => c.name)).toEqual(["Importante"]);
  });
