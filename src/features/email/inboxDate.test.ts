...
describe("formatInboxListDate", () => {
  ...
  it("includes the year for an older message", () => {
    ...
    expect(formatInboxListDate("2024-12-24T12:00:00Z", NOW)).toMatch(/24 de dez\. de 2024/);
  });
...
describe("formatReaderDate", () => {
  it("formats like Pipedrive: long month + day with a relative age, no seconds", () => {
    ...
    expect(out).toMatch(/^10 de junho \(há 29 dias\)$/);
    expect(out).not.toMatch(/:/); // no clock time / seconds
  });

  it("says (today) and (yesterday) for the most recent messages", () => {
    expect(formatReaderDate("2026-07-09T08:00:00Z", NOW)).toMatch(/\(hoje\)$/);
    expect(formatReaderDate("2026-07-08T08:00:00Z", NOW)).toMatch(/\(ontem\)$/);
  });

  it("includes the year for a message from a previous year", () => {
    expect(formatReaderDate("2024-12-24T12:00:00Z", NOW)).toMatch(/24 de dezembro de 2024/);
  });
...
describe("formatTimelineEmailDate", () => {
  ...
  it("shows time of day plus a relative age for today", () => {
    ...
    expect(out).toMatch(/\(há um minuto\)$/);
  });

  it("shows minutes for a message under an hour old", () => {
    expect(formatTimelineEmailDate("2026-08-18T16:54:00Z", now)).toMatch(/\(há 11 minutos\)$/);
  });

  it("shows hours for a message under a day old", () => {
    expect(formatTimelineEmailDate("2026-08-18T14:05:00Z", now)).toMatch(/\(há 3 horas\)$/);
  });

  it("shows days beyond that", () => {
    expect(formatTimelineEmailDate("2026-08-15T17:05:00Z", now)).toMatch(/\(há 3 dias\)$/);
  });
