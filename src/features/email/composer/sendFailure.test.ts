...
describe("sendFailureMessage", () => {
  ...
  test("a revoked Google grant names reconnecting, not retrying", () => {
    ...
    expect(msg).toMatch(/reconect/i);
    expect(msg).not.toMatch(/tente novamente/i);
  });

  test("an unreadable attachment points at the attachment", () => {
    expect(sendFailureMessage(ERROR_IDS.GMAIL_ATTACHMENT_DENIED)).toMatch(/anexo/i);
  });

  test("a dead session asks for a reload rather than a blind retry", () => {
    expect(sendFailureMessage(ERROR_IDS.PERM_DENIED)).toContain("Atualize a página");
  });

  test("a transient Gmail failure is the one case where retrying is the advice", () => {
    expect(sendFailureMessage(ERROR_IDS.GMAIL_API_EXHAUSTED)).toMatch(/novamente/i);
  });
