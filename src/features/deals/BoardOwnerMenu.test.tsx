...
describe("BoardOwnerMenu", () => {
  it("shows Everyone as the trigger label by default", () => {
    renderMenu();
    expect(screen.getByRole("button", { name: /Todos/ })).not.toBeNull();
  });
  ...
  it("closes the popover when Escape is pressed", async () => {
    ...
    renderMenu();
    await user.click(screen.getByRole("button", { name: /Todos/ }));
    expect(screen.getByPlaceholderText("Buscar responsável")).not.toBeNull();
    await user.keyboard("{Escape}");
    expect(screen.queryByPlaceholderText("Buscar responsável")).toBeNull();
  });
  ...
  it("filters the owner list via the search box", async () => {
    ...
    renderMenu();
    await user.click(screen.getByRole("button", { name: /Todos/ }));
    fireEvent.change(screen.getByPlaceholderText("Buscar responsável"), { target: { value: "bob" } });
  ...
  it("marks the signed-in user's row", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /Todos/ }));
    expect(screen.getByText("(eu)")).not.toBeNull();
  });
});
