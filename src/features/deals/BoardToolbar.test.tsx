...
describe("BoardToolbar view toggle", () => {
  it("renders Board and List as icon buttons (Pipedrive), with accessible names", () => {
    renderToolbar();
    const board = screen.getByRole("link", { name: "Visualização em quadro" });
    const list = screen.getByRole("link", { name: "Visualização em lista" });
  ...
  it("shows a visible text label on each segment so List/Archive are discoverable (P6)", () => {
    renderToolbar();
    expect(screen.getByRole("link", { name: "Visualização em quadro" }).textContent).toContain("Quadro");
    expect(screen.getByRole("link", { name: "Visualização em lista" }).textContent).toContain("Lista");
    expect(screen.getByRole("link", { name: "Visualização de arquivados" }).textContent).toContain("Arquivados");
  });

  it("links to the Edit Pipeline page from a pencil control", () => {
    renderToolbar();
    const edit = screen.getByRole("link", { name: "Editar pipeline" });
  ...
  it("defaults to the Board tab being current", () => {
    renderToolbar();
    expect(screen.getByRole("link", { name: "Visualização em quadro" }).getAttribute("aria-current")).toBe(
      "page",
    );
    expect(screen.getByRole("link", { name: "Visualização em lista" }).getAttribute("aria-current")).toBeNull();
  });

  it("marks the List tab current (and not Board) when the list view is active", () => {
    renderToolbar("list");
    expect(screen.getByRole("link", { name: "Visualização em lista" }).getAttribute("aria-current")).toBe(
      "page",
    );
    expect(
      screen.getByRole("link", { name: "Visualização em quadro" }).getAttribute("aria-current"),
    ).toBeNull();
  });

  it("marks the Archive tab current when the archived view is active", () => {
    renderToolbar("archived");
    expect(screen.getByRole("link", { name: "Visualização de arquivados" }).getAttribute("aria-current")).toBe(
      "page",
    );
    expect(
      screen.getByRole("link", { name: "Visualização em quadro" }).getAttribute("aria-current"),
  ...
  it("keeps the full board summary and pipeline controls on the list/archive views (parity)", () => {
    ...
    expect(screen.getByRole("link", { name: "Editar pipeline" })).not.toBeNull();
    expect(screen.getByText(/negóci/i)).not.toBeNull();
  });
});
